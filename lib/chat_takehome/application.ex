defmodule ChatTakehome.Application do
  # See https://elixir.hexdocs.pm/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      ChatTakehomeWeb.Telemetry,
      ChatTakehome.Repo,
      {DNSCluster, query: Application.get_env(:chat_takehome, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: ChatTakehome.PubSub},
      ChatTakehomeWeb.Presence,
      # Start a worker by calling: ChatTakehome.Worker.start_link(arg)
      # {ChatTakehome.Worker, arg},
      # Start to serve requests, typically the last entry
      ChatTakehomeWeb.Endpoint
    ]

    # See https://elixir.hexdocs.pm/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: ChatTakehome.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    ChatTakehomeWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
